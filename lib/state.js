...
function toolTarget(toolInput) {
  ...
  // Whitespace collapse first so newlines/tabs become single spaces, then strip control
  // characters (ESC included) so tool input can't inject terminal escapes into the status line.
  const oneLine = raw.replace(/\s+/g, ' ').trim().replace(CONTROL_CHARS, '');
